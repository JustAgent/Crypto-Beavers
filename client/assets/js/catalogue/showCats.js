...
function selectBreed(dna, id, gen, gender) {
    ...
    $('#' + gender).html(body)
    //3 Render the cats CSS style depending on DNA string
    renderCat(BeaverDna, gender)
    $('#' + gender).addClass('breedSelect')
    $('#' + gender).attr('data-catid', id)
    $('#' + gender).attr('onclick', 'breedbeavers("' + gender + '")')
    $('#catDNA' + gender).html(`
...
function removeSelection(id, gender) {
    ...
    if (gender == 'Dame') {
        ...
        if (catData == id) {
            ...
            $('#Sire').attr('onclick', 'breedbeavers(this.id)')
    ...
    if (gender == 'Sire') {
        ...
        if (catData == id) {
            ...
            $('#Dame').attr('onclick', 'breedbeavers(this.id)')
...
// Checks the Beaver on market situation
